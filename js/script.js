...
async function initButtonClick() {
  $('.ui.button').on('click', async function(e) {
    ...
    ethAmount = (tradeAmount*ethBalance)/(tokenBalance + tradeAmount) * (1+commission) * 1/1e18,
    buyPrice = (ethBalance / (tokenBalance + tradeAmount)) * (1/(Math.pow(10,18-tokenDecimals))),
    ...
    : (tradeAmount = inputValue * Math.pow(10,tokenDecimals), $('#tokenAmount').text(inputValue.toString()),
      console.log(tokenBalance,tradeAmount,ethBalance),			   
      sellPrice = (ethBalance / (tokenBalance + tradeAmount)) * (1/(Math.pow(10,18-tokenDecimals))),
